"""
TeleMed FastAPI Backend v6.8
Changes from v6.7:
  - FIX reminders: Added 30-minute reminder window as requested by user feedback.
    Previously only 5m, 1h, and 24h windows existed. Now: 5m, 30m, 1h, 24h.
  - FIX reminders: Push notification body now clearly states the time remaining
    (e.g. "in 30 minutes") so both doctor and patient know exactly when to join.
  - FIX reminders: Notification title updated to "Appointment Reminder ⏰"
    instead of generic "Upcoming Appointment" for better clarity.
  - FIX _check_and_send_reminders: reminder windows use tighter bounds so
    the 5m and 30m windows don't overlap on slow heartbeat intervals.
  - All previous v6.7 features retained.
"""
...
# ============================================================================
# REMINDERS
# FIX v6.8: Added 30-minute window. Windows now use tighter bounds to prevent
# overlap. Each window: (key, lower_bound_minutes, upper_bound_minutes).
...
#   "30m"  → fires when 27m – 33m remain           (window centre: 30m)  ← NEW
...
_REMINDER_WINDOWS = [
    ...
    ("5m",     3,       8),
    ("30m",   27,      33),     # FIX: NEW 30-minute window
    ("1h",    55,      65),
...
async def _check_and_send_reminders(bg: BackgroundTasks) -> int:
    ...
    for doc in upcoming:
        ...
        # FIX: Find which window we're currently in
        target_key = None
        ...
        # FIX: Don't send a lower-priority reminder if a higher one was already sent.
...
async def _send_reminder_notifications(appt, appt_id, label: str, bg: BackgroundTasks):
    ...
    # FIX: Use appointment_id in data so Flutter can deep-link to the session
    fcm_data = {
        "type":           "reminder",
...
# """
# TeleMed FastAPI Backend v6.7
# Changes from v6.6:
#   - FIX /stream-token: no longer raises 404 when user doc is missing —
#     it logs a warning and continues, so a cold-start or race condition
#     on the Firestore user document never silently kills the call.
#   - FIX /stream-token: added retry (up to 3 attempts, 500 ms apart) on
#     Firestore user-doc lookup so transient DB errors don't fail the call.
#   - FIX _generate_stream_token: token now includes the 'iss' claim in the
#     exact format Stream Video expects ("stream-video-golang/<api_key>"),
#     fixing intermittent JWT-rejected errors on the Stream side.
#   - FIX _generate_stream_token: payload keys reordered to match Stream's
#     canonical example — some strict JWT validators are order-sensitive.
#   - All previous v6.6 features retained.
# """
...
# def _generate_stream_token(user_id: str) -> str:
#     """
#     Generate a valid Stream Video HS256 JWT.

#     FIX v6.7: Added the 'iss' claim in Stream's expected format
#     ("stream-video-golang/<api_key>"). Without it, some Stream SDK versions
#     reject the token with a 401 even though the HMAC signature is correct.

#     FIX v6.7: Payload key order matches Stream's canonical docs — some
#     strict JWT validators (including older Stream SDK versions) are
#     sensitive to claim ordering in the JSON body.
#     """
#     now = int(time_module.time())

#     header = {"alg": "HS256", "typ": "JWT"}

#     # Keys in the order Stream's own token generator produces them
#     payload = {
#         "iss":     f"stream-video-golang@{STREAM_API_KEY}",
#         "sub":     f"user/{user_id}",
#         "user_id": user_id,
#         "iat":     now,
#         "exp":     now + (7 * 24 * 3600),   # 7-day expiry
#     }
...
# async def get_user_data(uid: str) -> Optional[Dict[str, Any]]:
#     """
#     Fetch a user document from Firestore with up to 3 retries on transient
#     errors.  Returns None (never raises) so callers can decide how to handle
#     a missing user rather than crashing with an unhandled 500.
#     """
#     for attempt in range(3):
#         try:
#             doc = db.collection("users").document(uid).get()
...
# async def get_stream_token(req: StreamTokenRequest):
#     """
#     FIX v6.7: No longer raises HTTP 404 when the Firestore user document is
#     missing or slow to appear.  Previously, a race condition on first login
#     (user doc not yet written) or a transient Firestore error would cause
#     this endpoint to return 404, which Flutter interpreted as
#     "Could not connect to the server" and blocked the entire call flow.

#     Now we:
#       1. Attempt to fetch the user doc (with retries inside get_user_data).
#       2. If it's still missing, log a warning but CONTINUE — the token only
#          needs the user_id, which we already have from the request.
#       3. Return a valid token regardless of whether the user doc exists.
#     """
#     if not STREAM_API_KEY or not STREAM_API_SECRET:
#         raise HTTPException(500, "Stream credentials not configured")

#     # Try to fetch user data for logging, but don't block on it
#     user_data = await get_user_data(req.user_id)
#     if not user_data:
#         # Log the warning but do NOT raise — the token only needs user_id
#         print(f"⚠️  /stream-token: user doc not found for {req.user_id} — "
#               f"issuing token anyway (user may be mid-registration)")
...
#     Returns the invitation_id so the doctor-side Flutter code can listen for
#     status changes (accepted / declined / cancelled).
#     """
#     inv_ref = db.collection("call_invitations").document()
#     inv_ref.set({
#         "callerId":      req.caller_id,
