...
Changes for backend v7.0 (session lifecycle integration):
  - _on_payment_success now, right after writing the appointment, calls
    payment_store.seed_lifecycle_fields(payment_ref, appointment_id) to
    initialise the `lifecycle_status` / `current_appointment_id` fields on
    the SAME payments/{payment_ref} document payment_store already owns.
    No new document or collection is created — earlier drafts of this
    integration mistakenly created a second record with an auto-generated
    ID in the same `payments` collection, which collided with
    payment_store's own payment_ref-as-doc-ID scheme. Fixed here.
  - appointments/{id}.linkedPaymentId now stores the payment_ref STRING
    itself (the real payments/ document ID), so
    main.py's session-lifecycle endpoints can update the correct record
    directly via db.collection("payments").document(linkedPaymentId).
  - _on_payment_success now guards against double-processing: if an
    appointment already exists for this payment_ref (webhook fired twice,
    or the status-poll fallback raced the webhook), it exits early instead
    of writing a second appointment for the same charge.
  - No other behavior changed. payment_store's own pending/successful/
    failed bookkeeping, fapshi module, and the initiate/webhook/status
    contracts with Flutter are all unchanged.
...
POST /payment/webhook
    ...
    (linked to the existing payment record) and sends FCM + email
    notifications. On FAILED: marks the payment record as failed so
    Flutter shows an error.
...
Flutter → POST /payment/initiate
         ...
         Fapshi → POST /payment/webhook  (async, usually within seconds)
             ↓
         Backend writes Firestore appointment + seeds lifecycle fields on
         the existing payment record + sends FCM + email
             ↓
...
async def initiate_payment(req: InitiatePaymentRequest):
    ...
    # NOTE: linkedPaymentId is intentionally NOT set here — the appointment
    # doesn't exist yet, and the payment_ref that will BECOME linkedPaymentId
    # is already known (it's `payment_ref` below), but we deliberately set
    # it explicitly at write time in _on_payment_success() rather than here,
    # to keep "this payload becomes the appointment" a single obvious
    # transformation in one place. lastReminderSent is "" (not None) for
    # consistency with how /appointment/reschedule resets it in main.py.
    appointment_payload = {
...
async def _on_payment_success(payment_ref: str) -> None:
    ...
    Steps:
      ...
      2. Idempotency guard: if an appointment already exists for this
         payment_ref (webhook fired twice, or the status-poll fallback
         raced the webhook), stop here instead of writing a duplicate
         appointment for the same charge.
      3. Mark payment successful in payment_store (Fapshi bookkeeping,
         unchanged — status field only).
      4. Write the appointment document to Firestore, with
         linkedPaymentId = payment_ref.
      5. Seed the lifecycle fields (lifecycle_status,
         current_appointment_id) on the SAME payments/{payment_ref} doc —
         no second document created.
      6. Send FCM push notifications to patient and doctor.
    ...
    # ── 2. Idempotency guard against duplicate appointment creation ─────────
    if _appointment_exists(payment_ref):
        logger.info(
            "↩️  _on_payment_success: appointment already exists for ref=%s — skipping",
            payment_ref,
        )
        payment_store.mark_payment_successful(payment_ref)
        return

    # ── 3. Mark payment successful (Fapshi status only) ──────────────────────
    payment_store.mark_payment_successful(payment_ref)

    payload = dict(record["appointment_payload"])  # copy — we mutate below
    ...
    firestore_doc = {
        **payload,
        ...
        "linkedPaymentId": payment_ref,   # the REAL payments/ doc ID — no new record
        "createdAt":       firestore.SERVER_TIMESTAMP,
    }
    apt_ref.set(firestore_doc)

    logger.info(
        "📅 Appointment written: id=%s patient=%s doctor=%s linkedPaymentId=%s",
        appointment_id, payload["patientId"], payload["doctorId"], payment_ref,
    )

    # ── 5. Seed lifecycle fields on the existing payment record ─────────────
    payment_store.seed_lifecycle_fields(payment_ref, appointment_id)
    ...
    if current_status == "pending" and fapshi_trans_id:
        try:
            ...
            if fresh_status == "SUCCESSFUL":
                ...
                # _on_payment_success() itself guards against double-write
                # via _appointment_exists(), so this is safe even if the
                # webhook lands a moment later.
